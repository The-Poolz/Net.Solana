...
using Net.Solana.Rpc.Converters;
using Net.Solana.Rpc.Core;
using Net.Solana.Rpc.Core.Http;
using Net.Solana.Rpc.Core.Sockets;
using Net.Solana.Rpc.Messages;
using Net.Solana.Rpc.Models;
using Net.Solana.Rpc.Types;

namespace Net.Solana.Rpc;
...
public class SolanaStreamingRpcClient : StreamingRpcClient, IStreamingRpcClient
{
    ...
    /// <summary>
    /// Maps the public ids to the unconfirmed subscription state objects.
    /// </summary>
    private readonly Dictionary<int, SubscriptionState> _unconfirmedRequests = new Dictionary<int, SubscriptionState>();

    /// <summary>
    /// Maps the server ids to the confirmed subscription state objects.
    /// </summary>
    private readonly Dictionary<int, SubscriptionState> _confirmedSubscriptions = new Dictionary<int, SubscriptionState>();
    ...
    public SolanaStreamingRpcClient(string url, ILogger? logger = null, IWebSocket? websocket = default, ClientWebSocket? clientWebSocket = default) : base(url, logger, websocket, clientWebSocket)
    {
    }
    ...
    private SubscriptionState RemoveUnconfirmedSubscription(int id)
    {
        ...
        lock (this)
        {
            if (!_unconfirmedRequests.Remove(id, out sub))
            {
    ...
    private void RemoveSubscription(int id, bool shouldNotify)
    {
        ...
        lock (this)
        {
            if (!_confirmedSubscriptions.Remove(id, out sub))
            {
    ...
    /// <summary>
    /// Confirms a given subcription based on the public subscription id and the newly received external id.
    ...
    private void ConfirmSubscription(int internalId, int resultId)
    {
        ...
        lock (this)
        {
            if (_unconfirmedRequests.Remove(internalId, out sub))
            {
