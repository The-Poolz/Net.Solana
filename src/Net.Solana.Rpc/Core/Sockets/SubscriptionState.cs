...
public abstract class SubscriptionState
{
    ...
    public void ChangeState(SubscriptionStatus newState, string error = null, string code = null)
    {
        State = newState;
        LastError = error;
        LastCode = code;
...
public class SubscriptionState<T> : SubscriptionState
{
    ...
    /// <param name="additionalParameters">Additional parameters for this given subscription.</param>
    public SubscriptionState(SolanaStreamingRpcClient rpcClient, SubscriptionChannel chan, Action<SubscriptionState, T> handler, IList<object> additionalParameters = default)
